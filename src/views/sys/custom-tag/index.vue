  ...
  <div class="p-2">
        ...
        <el-card shadow="hover">
          <el-form ref="queryFormRef" :model="queryParams" :inline="true" @submit.prevent>
            <el-form-item label="标签名称" prop="label">
              <el-input v-model="queryParams.label" placeholder="请输入标签名称" clearable @keyup.enter="handleQuery" />
            </el-form-item>
            <el-form-item>
    ...
    <el-card shadow="hover">
      ...
      <el-table v-loading="loading" :data="tableData" tooltip-effect="dark myTooltips" @selection-change="handleSelectionChange">
        ...
        <el-table-column label="标签名称" align="center" prop="label" width="150" />
...
<script setup name="customTag" lang="ts">
import { configTagList, configTagAdd, configTagDel, configTagInfo, configTagUp } from '@/api/sys/custom-tag';
import { FormData, TableQuery, TableVO } from '@/api/sys/custom-tag/types';
...
const getTableData = async () => {
  loading.value = true;
  const res = await configTagList(queryParams.value);
...
const handleUpdate = async (row?: TableVO) => {
  ...
  const res = await configTagInfo(postId);
...
const submitForm = () => {
  FormDataRef.value?.validate(async (valid: boolean) => {
    if (valid) {
      form.value.id ? await configTagUp(form.value) : await configTagAdd(form.value);
