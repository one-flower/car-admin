<template>
  <div class="p-2">
        ...
        <el-card shadow="hover">
          <el-form ref="queryFormRef" :model="data.queryParams" :inline="true" @submit.prevent>
            ...
            <el-form-item label="在职状态" prop="state">
              <el-select v-model="data.queryParams.state" value-key="" placeholder="请选择在职状态" clearable filterable>
                <el-option v-for="item in clyh_staff_entry_state" :key="item.value" :label="item.label" :value="item.value"> </el-option>
              </el-select>
            </el-form-item>
            <el-form-item>
    ...
    <el-card shadow="hover">
      <template #header>
        <el-row :gutter="10" class="mb8">
          <el-col :span="1.5">
            <el-button v-hasPermi="['clyh:staff:add']" type="primary" plain icon="Plus" @click="handleAdd">新增</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button v-hasPermi="['clyh:staff:remove']" type="danger" plain icon="Delete" :disabled="tableAttr.multiple" @click="handleDelete()">
              删除
            </el-button>
          </el-col>
      ...
      <el-table v-loading="loading" :data="tableData" tooltip-effect="dark myTooltips" @selection-change="handleSelectionChange">
        ...
        <el-table-column label="在职状态" align="center" prop="stateLabel" />
        <el-table-column label="员工简介" align="left" header-align="center" prop="snapshot" show-overflow-tooltip />
        <el-table-column label="操作" width="180" align="center" class-name="small-padding fixed-width">
          <template #default="{ row }">
            <el-tooltip content="提成明细" placement="top">
              <el-button v-hasPermi="['clyh:staff:edit']" link type="warning" @click="handleCommExt(row)">
                <svg-icon class-name="search-icon" icon-class="commission"></svg-icon>
              </el-button>
            </el-tooltip>
            <el-tooltip content="修改" placement="top">
              <el-button v-hasPermi="['clyh:staff:edit']" link type="primary" icon="Edit" @click="handleUpdate(row)"></el-button>
            </el-tooltip>
            <el-tooltip content="删除" placement="top">
              <el-button v-hasPermi="['clyh:staff:remove']" link type="danger" icon="Delete" @click="handleDelete(row)"></el-button>
            </el-tooltip>
            <el-tooltip content="详情" placement="top">
              <el-button v-hasPermi="['clyh:staff:query']" link type="info" @click="handleDetail(row)">
      ...
      <el-form ref="FormDataRef" :model="data.form" :rules="data.rules" label-width="80px" :disabled="formDetail" @submit.prevent>
        ...
        <el-form-item label="性别" prop="gender">
          <el-radio-group v-model="data.form.gender" :disabled="data.form.id !== undefined">
            <el-radio v-for="item in sys_user_sex" :key="item.key" :label="item.label" :value="item.value"> </el-radio>
          </el-radio-group>
        </el-form-item>
        <el-form-item label="身份证号" prop="cardNum">
          <el-input v-model="data.form.cardNum" placeholder="请输入身份证号" :disabled="data.form.id !== undefined" />
        </el-form-item>
        <el-form-item label="联系电话" prop="telephone">
        ...
        <el-form-item label="在职状态" prop="state">
          <el-radio-group v-model="data.form.state">
            <el-radio v-for="item in clyh_staff_entry_state" :key="item.key" :label="item.label" :value="item.value"> </el-radio>
          </el-radio-group>
        </el-form-item>
    ...
    <comm-ext v-model:visible="commDialog.visible" :pesron-info="data.form"></comm-ext>
  </div>
</template>
...
import CommExt from './comm-ext.vue';
...
const commDialog = reactive<DialogOption>({
  visible: false,
  title: '提成明细'
});
